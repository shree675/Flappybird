...
while not done:
    ...
    if e==0:
        screen.blit(text3,(300-text3.get_width()//2,250-text3.get_height()//2))
        pygame.display.flip()
    if (stop2-start1)>=4:
        e=1
    if w==1 and press[pygame.K_RETURN]: done=True
    if w==0 and e==1: 
        stop1=time.time()
        for k in range(a):
            v[k]-=1.5
        ...
        if r==0 and y<475:
            ...
            h+=0.1
        for k in range(a):
            pygame.draw.rect(screen,(10,200,10),pygame.Rect(v[k],l[k],60,400))
            pygame.draw.rect(screen,(10,200,10),pygame.Rect(v[k],490+l[k],60,400))
            pygame.draw.rect(screen,(10,220,10),pygame.Rect(v[k]-2,370+l[k],64,30))
            pygame.draw.rect(screen,(10,220,10),pygame.Rect(v[k]-2,490+l[k],64,30))
        if y>=475:
            y=475
            w=1
            sound3.stop()
            sound2.play(0)
            screen.blit(text1,(300-text1.get_width()//2,250-text1.get_height()//2))
